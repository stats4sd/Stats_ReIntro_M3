...
-   What a statistical hypothesis is, and the concept of the null hypothesis.
...
-   We obtained the *p-value = 0.00022*, which is the probability of obtaining a value as extreme as this if the null hypothesis is true. This is the *two-sided test*.
...
2- Choose a test statistic that estimates something which, if it turns out to be extreme enough, would lead us to doubt the null hypothesis. You will not actually have to calculate this statistic yourself, as each (standard) statistical test has already defined the relevant test statistic.

3- Generate the sampling distribution of this test statistic, assuming the null hypothesis is true.

4- Obtain the p-value by comparing your test statistic with the sampling distribution. The p-value is the probability of observing such an extreme statistic if the null hypothesis is true. When setting up the alternative hypothesis we decided whether to use a one-sided or two-sided p-value. In most tests, we will use the two-sided p-value.

5- Declare the result significant if the p-value is below some critical threshold, α.
...
Although the confidence interval is a tool for estimation, we can also use it for hypothesis testing or to support it. Instead of generating the sampling distribution under the null hypothesis for the test statistic and comparing the test statistic with this distribution to obtain the p-value (steps 3 and 4 in the hypothesis testing process above), we can generate the confidence interval for the statistic, and observe whether the null hypothesis value lies within its limits or outside them.
...
Example: we want to test whether the mean length has changed over time. To do so, we are going to split the catches in the following four periods: *1949-1954*"*, *1955-1959*"*, and *1960-1963*. The figure below shows the histogram of the sample lengths for the four periods.


```{r}
Whale$period2<-ifelse(Whale$Year<1955, "1949-1954",
                        ifelse(Whale$Year<1960, "1955-1959","1960-1963"))

ggplot(Whale, aes(x=length.m., group = period2))+
  geom_histogram(bins=10)+
  labs(x="Length(m)")+
  facet_wrap(~period2,nrow =3)
...
-   ANOVA: p-value = $2.2*10^{-16}$, it is very small, therefore we can reject the null hypothesis  at any of the usual significant levels,  α.
...
```{r}
model_ANOVA<-lm(length.m.~period2, data=Whale)
anova(model_ANOVA)

emmeans(model_ANOVA,~period2)
```
...
-   We are more likely to detect a difference by rejecting the null hypothesis the larger the sample size is 
 
-   At the same time, for a given sample size, the smaller the difference we want to be able to detect, the more difficult it will be to reject the null hypothesis. 
 
-   In addition, the variability of our data also plays a role in the tests: given a sample size and a difference we want to be able to detect, the larger the variability of our data is, the more difficult it will be to reject the null hypothesis. 
 
-   Finally, if all of the above are fixed, the minimum sample size depends on the sampling scheme used. It will be smallest for a simple random sampling design and larger for other designs, with how much larger depending on the particular design used.

Therefore, we can determine our sample size as the smallest one that will allow us to reject the null hypothesis, given the minimum difference we would like to be able to detect. To do this, we need to define this minimum detectable difference and obtain an estimate of the population measure’s variance.

We can also determine the minimum sample size required to detect a difference in proportions. The only difference is that the minimum sample size depends on the proportion $p$, being maximal at 0.5 and decreasing as it approaches the limits of 0 or 1. Therefore, we need an initial estimate of the proportion $p$ that we want to estimate and/or test.

Given all the assumptions we need to make, determining the minimum sample size can be difficult, and the suggestion is to use the entire available budget for data collection.


## What p-values can and can't do
...
The p-value is small enough (0.004) for us to confidently reject the null hypothesis. Right? well, maybe wrong!

Now let's carry out the experiment in a slightly different way: 1024 people are given a fair coin, and they flip it. Those that get heads remain, those that get tails, leave the room. We expect about 50% staying and 50% leaving, about 512. Then we repeat the process a second time, and a third, and a fort, up to a tenth time. We expect one person to still be in the room at the end of this process. 

As you can see with the example, the p-value sometimes can be misleading. and it is not just that it's definition is a bit convoluted. Even assuming that we agree with the definition, if you run enough experiment you can always get a p-value that demonstrates what you wanted to demonstrate, just by random chance.

There are criticisms about the use of p-value, there is a whole approach to statistics, the Bayesian approach, that plainly rejects them. In 2016, the American Statistical Association managed to get a group of statisticians to agree on six principles about p-values:
...
The significance levels threshold are, as we have seen, arbitrary. A *significant* result does not necessarily mean that we have proven discovery.

4. Proper inference requires full reporting and transparency

In the example, we have seen that if you repeat the experiment enough times, you will end up with a significant result, even if the null hypothesis is true. We should report our sampling scheme, make data available, describe the methods used so others can replicate our experiment.

5. A p-value does not measure the size of an effect or the importance of a result

A significant result tells us that the observed difference is unlikely to be due to random chance, but desn't tell us anything about how large or small this difference is.
