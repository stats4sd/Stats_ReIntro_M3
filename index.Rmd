...
The figure below shows the 95% confidence interval for the mean length of the whales in 1949. Two lines show two null hypothesis we can test: 13.0 and 13.2. 13.0 is included within the limits of the 95% confidence interval, we therefore cannot reject this null hypothesis. However, 13.2 lays outside the limits, therefore we would reject this null hypothesis and the results are significant at the α = 0.05 level. You can check this results in the interactive 1-sample t-test in the next section.
...
Now, let's carry out the experiment in a slightly different way: 1024 people are given a fair coin, and they flip it. Those that get heads remain, those that get tails, leave the room. We expect about 50% staying and 50% leaving, about 512. Then we repeat the process a second time, and a third, and a fourth, up to a tenth time. We expect one person to still be in the room at the end of this process. 

As you can see with the example, the p-value sometimes can be misleading. And it is not just that it's definition is a bit convoluted. Even assuming that we agree with the definition, if you run enough times an experiment, you can always get a p-value that demonstrates what you wanted to demonstrate, just by random chance.
...
In the example above when 1024 people flipped 10 times a coin, we have seen that if you repeat the experiment enough times, we will end up with a significant result, even if the null hypothesis is true. We should report our sampling scheme, make data available, describe the methods used so others can replicate our experiment, and confirm whether there is an effect or our result was due to chance.

5. A p-value does not measure the size of an effect or the importance of a result

A significant result tells us that the observed difference or effect is unlikely to be due to random chance, but does not tell us anything about how large or small this difference (or effect) is.
...
The p-value depends on how far from 0 is $\frac{|estimate|}{SE}$. For example, Education has an estimate that is less than the Natural risk disaster, but its p-value is smaller too, because the SE is also smaller. 
...
we want to test the hypothesis that the mean length in humpback wales is equal for males and females. We have the data collected in 1962 as sample, that we can consider is representative of the population of humpback whales.
...
```{r}
t.test(length.m.~sex, data=Whale_62, alternative="two.sided")
...
```{r}
t.test(length.m.~sex, data=Whale_62, alternative="less")
...
```{r}
t.test(length.m.~sex, data=Whale_62, alternative="greater")
